module.exports = (client, message) => { // Function for when a message is sent. "message" = the message object of said new message.
  if(!message.content.startsWith(client.config.prefix)) return; // Stop if the message doesn't start with the specified prefix.
  ...
  const args = message.content.slice(client.config.prefix.length).trim().split(/ +/g); // Sets "args" to the seperate arguments, Ex: message: "Hello there world!" will be turned into ["Hello", "there", "world!"].
  ...
  if(!cmd) return; // This will make it so that if the bot doesn't have the specified command, it won't run it. Obviously..
